...
let game (ret : ILeducGame<'r>) : 'r =
    ...
    let rec round_two is_call_check raises_left (p1,p2) community_card =
        ret.action_round_two(p1, p2, raises_left, community_card, function
        | Fold -> ret.terminal_fold(p1, p2.id, p1.pot)
        | Call when is_call_check -> round_two false raises_left (p2,p1) community_card
        | Call ->
    ...
    let rec round_one is_call_check raises_left (p1, p2) =
        ret.action_round_one(p1, p2, raises_left, function
        | Fold -> ret.terminal_fold(p1, p2.id, p1.pot)
        | Call when is_call_check -> round_one false raises_left (p2,p1)
        | Call ->
            let p1 = {p1 with pot=p2.pot}
