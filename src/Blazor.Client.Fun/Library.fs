...
type AllowedActions =
    ...
    // Previously this was static to make the type serializable, but we don't need to care about anymore.
    member q.IsAllowed act =
        match act with
        | Fold -> q.is_fold
        | Call -> q.is_call
        | Raise -> q.is_raise
...
module Reducers =
    ...
    type MsgServerToClient = unit // We'll leave this for later.
    type MsgClient =
    ...
    // Mhhh, no, we need the UI messages.
    // Damn, there is a shitload of these.
    // It is trying to handle a bunch of web.
    // Should I add a dependency on the Leduc stuff?
