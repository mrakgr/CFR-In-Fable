...
module Html =
    module prop =
        let dataTarget (x : string) = prop.custom("data-target", x)
        let dataToggle (x : string) = prop.custom("data-toggle", x)
        let ariaHaspopup (x : bool) = prop.custom("aria-haspopup",x)
        let ariaLabelledby (x : string) = prop.custom("aria-labelledby",x)
